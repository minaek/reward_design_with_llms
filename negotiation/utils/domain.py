...
class ObjectDivisionDomain(Domain):
    ...
    def parse_human_choice(self, inpt, output):
        ...
        for x, n in zip(choice, cnts):
            if x < 0 or x > n:
                raise ValueError
        my_choice = ['item%d=%d' % (i, x) for i, x in enumerate(choice)]
        their_choice = ['item%d=%d' % (i, n - x) for i, (x, n) in enumerate(zip(choice, cnts))]
        return my_choice + their_choice
    ...
    def score_choices(self, choices, ctxs):
        ...
        if "<no_agreement>" in choices[0] or "<no_agreement>" in choices[1] or "no_agreement" in choices[1]:
            return False, [0 for _ in range(len(ctxs))]
