...
class Agent(object):
    ...
    def read(self, inpt: List[str], **kwargs):
    ...
    def update(self, agree: bool, reward: float):
...
class LstmAgent(Agent):
    ...
    def _decode(self, out, dictionary):
        """A helper function that decodes indices into English words.
    ...
    def write(self):
        ...
        for _ in range(self.ncandidate):
            ...
            is_selection = len(move) == 1 and \
                           self.model.word_dict.get_word(move.item()[0]) == '<selection>'
...
class BatchedRolloutAgent(LstmRolloutAgent):
    ...
    def write(self):
        ...
        for i in range(self.args.rollout_bsz):
            ...
            dialog_words = Variable(self.model.to_device(outs.narrow(0, 0, eod_pos + 1))).to(device)
        ...
        for threshold in range(self.args.rollout_count_threshold, -1, -1):
            ...
            if cands:
                ...
                self.words.append(Variable(move).to(device))
                assert (torch.cat(self.words).size()[0] == torch.cat(self.lang_hs).size()[0])
...
class RlAgent(LstmAgent):
    ...
    def __init__(self, model, args, name='Alice', train=True, rewarder=None):
    ...
    def write_detailed(self, max_words=100) -> List[str]:
        """
        Additionally returns probability of chosen token and the output of the decoder
        """
        logprobs, outs, self.lang_h, lang_hs, prob, out = self.model.write(self.lang_h, self.ctx_h,
                                                                           max_words, self.args.temperature,
                                                                           self.context_mask)
        ...
        self.words.append(outs)
        assert (torch.cat(self.words).size()[0] == torch.cat(self.lang_hs).size()[0])
        return self._decode(outs, self.model.word_dict), prob, out
    ...
    #def write_partner(self, partner_type, max_words=1) -> List[str]:
    #    logprobs, outs, self.lang_h, lang_hs, _, _ = self.model.write(self.lang_h, self.ctx_h,
    #                                                                  max_words, self.args.temperature,
    #                                                                  self.context_mask, partner_type)
    #    # append log probs from the generated words
    #    self.logprobs.extend(logprobs)
    #    self.lang_hs.append(lang_hs)
    #    # first add the special 'YOU:' token
    ...
    #def read_partner(self, inpt, partner_type, prefix_token: str = 'THEM:'):
    #    inpt = self._encode(inpt, self.model.word_dict)
    #    lang_hs, self.lang_h = self.model.read(Variable(inpt).to(device), self.lang_h, self.ctx_h, partner_type,
    #                                           prefix_token=prefix_token)
    #    # append new hidden states to the current list of the hidden states
    #    self.lang_hs.append(lang_hs.squeeze(1))
    #    # first add the special 'THEM:' token
    ...
    def _get_choice(self, i, sent, agent_order, ctxs):
        # see who this proposal was proposed by
        propose_agent = agent_order[i]
        # format proposal
        cnts = [int(x) for x in ctxs[0][0::2]]
        proposal = sent[0].split(" ")[1:]
        other_proposal = [f'item{i}=' + str(cnts[i] - int(proposal[i][-1])) for i in range(len(cnts))]
        if propose_agent == 'Alice':
            if self.name == 'Alice':  # If I am Alice and I proposed this
                choice = proposal + other_proposal
            elif self.name in ['Human', 'Bob', 'Expert']:  # I am Human and Alice proposed
                choice = other_proposal + proposal
            else:
                raise ValueError
        elif propose_agent in ['Human', 'Bob', 'Expert']:
            if self.name in ['Human', 'Bob', 'Expert']:
                choice = proposal + other_proposal  # I am human and I proposed this
    ...
    def choose(self, conv, agent_order, ctxs, handcode='partial') -> List[str]:
        ...
        # make sure last element of conv is the <selection>
        if len(conv[-1]) == 6:  # means human has already entered selection
            del conv[-1]
            del agent_order[-1]
    ...
    def choose_handcoded(self, conv, agent_order, ctxs) -> List[str]:
        # loop through conv in reverse order and find the last suggested proposal
        # print("\n&&&&&&&&conv, ", conv)
        # print("agent order: ", agent_order)
        # print("&&&&&&")
        disagree = None
        choice = None
        assert conv[-1] == '<selection>'
        for i, sent in reversed(list(enumerate(conv))):
            if 'disagree' in sent:  # keep track of whether agents disagreed after final proposal
                disagree = True
            if 'propose' in sent[0] or 'insist' in sent[
                0]:  # if agents disagreed after final proposal, then no agreement
                if disagree:
                    choice = ["<no_agreement>"] * 6
                else:  # otherwise, choose final proposal
                    choice = self._get_choice(i, sent, agent_order, ctxs)
                return choice
...
class HumanAgent(Agent):
    ...
    def __init__(self, domain, args, name='Human', cda=False):
        self.name = name
        self.human = True
        self.domain = domain
        self.args = args
    ...
    def _is_numeric(self, inpt):
        return all([i.isnumeric() for i in inpt])

    def _is_valid_cda(self, inpt):
        return len(inpt) == 1 and inpt[0] in ['<selection>', 'agree', 'disagree']

    def write(self):
        while True:
            try:
                if self.cda:
                    return self._write_cda()
                else:
                ...
                print(
                    'Your sentence is invalid!\nPlease choose a valid response: {propose x y z, insist x y z, agree, disagree, <selection>}')

    def _write_cda(self):
        inpt = input('%s : ' % self.name).lower().strip().split()
        if len(inpt) == 3 and self._is_numeric(inpt):
            inpt = ["propose: item0={} item1={} item2={}".format(inpt[0], inpt[1], inpt[2])]
        elif len(inpt) == 4 and inpt[0] == 'propose' and self._is_numeric(inpt[1:]):
            inpt = ["propose: item0={} item1={} item2={}".format(inpt[1], inpt[2], inpt[3])]
        elif len(inpt) == 4 and inpt[0] == 'insist' and self._is_numeric(inpt[1:]):
            inpt = ["insist: item0={} item1={} item2={}".format(inpt[1], inpt[2], inpt[3])]  # add insist
        elif inpt[0] == 'e' or inpt[0] == 'end':
            inpt = ['<selection>']
        elif not self._is_valid_cda(inpt):
            raise RuntimeError
        return inpt
    ...
    def choose(self):
        while True:
            try:
                ...
                if choice.lower().strip() == 'no agreement':  # adding option for no_agreement
                    return ['<no_agreement>' for _ in range(6)]
                return self.domain.parse_human_choice(self.ctx, choice)
            except KeyboardInterrupt:
                sys.exit()
            except:
                print('Your choice is invalid! Try again. Choose a valid selection or type "no agreement".')
